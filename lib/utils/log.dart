...
class Log {
  ...
  static const String tag = "YD-LOG";
  ...
  static void _printList(List list, {String tag = tag, int tabs = 1}) {
    list.asMap().forEach((index, value) {
      ...
      if (value is Map) {
        if (value.isEmpty) {
          LogUtil.v('${_indent(tabs)} $value${isLast ? '' : ','}', tag: tag);
        } else {
          _printMap(value, tabs: tabs + 1, isListItem: true, isLast: isLast);
        }
      } else {
        LogUtil.v('${_indent(tabs + 2)} $e${isLast ? '' : ','}');
      }
    });
  }
